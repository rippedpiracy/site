...
const BASE_DOMAIN = process.env.CF_PAGES_URL ?? "ripped.guide";
// add domain
...
for (const file of files) {
  ...
  let parsed;
  try {
    parsed = await readFile(join(root, `${file}.mdx`), "utf-8").then(matter);
  } catch (e) {
    parsed = await readFile(join(root, `${file}.md`), "utf-8").then(matter);
  }
  const name =
    parsed.data.name ??
    parsed.data.title ??
